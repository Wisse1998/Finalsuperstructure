...
def Superstructure_model(Superstructure):
    ...
    model.Feedflow = Param(initialize = 32821, doc = ' Total feed flow fixed value kg/hr (sum of supply load')
    ...
    #model.y = Var(model.j, model.k, bounds = (0.0, 1), doc = 'Logic variable')
    ...
    model.Results = Var(model.g, bounds=(0.0, None), doc = 'RMC TAR AOC AIC UTC TotTrCost')
    ...
    def logic_rule(model, j):
        ...
        if j == 6:
            return sum(model.y[j,k] for k in model.k) == 0
        else:
            return Constraint.Skip
    ...
    def logic_rule1(model):
       return model.y[1,1] + model.y[1,2] + model.y[1,3] + model.y[1,4] + model.y[1,5] - model.y[2,1] - model.y[2,2] - model.y[2,3] == 0
    ...
    def Transport_rule2(model,d,p,j,k):
        if j == 6:
            return model.Transport[d,p] <= model.Demand[d,p]
        else:
            return Constraint.Skip
    
    def Transport_rule3(model, k, d, p):
            return model.TransportCost[d,p] == (model.Transport[d,p] * 1000 / model.TankLoad) * 2 * model.Marketdist[d,6] * model.FuelCon * model.FuelPrice
    
    def Transport_rule4(model):
    ...
    model.Transport_rule2 = Constraint(model.d, model.p , model.j, model.k, rule = Transport_rule2)
    model.Transport_rule3 = Constraint(model.k, model.d, model.p, rule = Transport_rule3)
    model.Transport_rule4 = Constraint(rule = Transport_rule4)
    ...
    "This one is working for only the first range------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    ...
    def EQ_rule4(model, j, k, u):
        if j == 4:
            if k == 1:
                ...
                return model.EQCost[j,k] == (model.x[1] * (0.1146e6 * (0/230) ** (0.75)) + model.x[2] * (0.1146e6 * (100000/230) **(0.75)) + model.x[3] * (0.1146e6 * (400000/230) ** (0.75)) + model.x[4] * (0.1146e6 * (600000/230) ** (0.75))) #* model.y[j,k]          
            if k == 2:
                return model.EQCost[j,k] == (model.x[1] * (1.5543e6 * (0/3296) ** (0.75)) + model.x[2] * (1.5543e6 * (100000/3296) **(0.75)) + model.x[3] * (1.5543e6 * (400000/3296) ** (0.75)) + model.x[4] * (1.5543e6 * (600000/3296) ** (0.75)))# * model.y[j,k]
            if k == 3:
                return model.EQCost[j,k] == (model.x[1] * (1.3485e6 * (0/4600) ** (0.75)) + model.x[2] * (1.3485e6 * (100000/4600) **(0.75)) + model.x[3] * (1.3485e6 * (400000/4600) ** (0.75)) + model.x[4] * (1.3485e6 * (600000/4600) ** (0.75)))# * model.y[j,k]
            if k == 4:
                return model.EQCost[j,k] == (model.x[1] * (4.8155e6 * (0/8500) ** (0.75)) + model.x[2] * (4.8155e6 * (100000/8500) **(0.75)) + model.x[3] * (4.8155e6 * (400000/8500) ** (0.75)) + model.x[4] * (4.8155e6 * (600000/8500) ** (0.75)))# * model.y[j,k]
            else:
                return model.EQCost[j,k] == 0
        else:
            return Constraint.Skip
    ...
    "Not applicable for the glpk solver optimizaiton"
...
#         if j == 3:
#             if k == 2 or k == 3:
#                 return model.Util[j,k,u] == (model.Feedflow - model.flowstage[j-1,1]) * model.Tau[j,k,u] / 1000 * model.y[j,k]
#             else:
#                 return model.Util[j,k,u] == model.Feedflow * model.Tau[j,k,u] / 1000 * model.y[j,k]
#         if j == 4:
#             return model.Util[j,k,u] == sum(model.flowstage[j-1,i] for i in model.i) * model.Tau[j,k,u] / 1000 * model.y[j,k]
#         else:
...
# =============================================================================
    ...
    "Annualized cost flows calculations---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    ...
    def Results_rule(model, g):
        if g == 1:
            return model.Results[g] == model.AOC
        elif g == 2:
            return model.Results[g] == model.AIC 
        elif g == 3:
            return model.Results[g] == model.TAR 
        elif g == 4:
            return model.Results[g] == model.RMC
        elif g == 5:
            return model.Results[g] == model.TUC 
        elif g == 6:
            return model.Results[g] == model.TotTrCost + model.TotTransportCost
        elif g == 7:
            return model.Results[g] == model.TotalTIC 
        elif g == 8:
            return model.Results[g] == model.TotalTDC 
        elif g == 9:
            return model.Results[g] == sum(model.TotSECCost[j,k,i] for j in model.j for k in model.k for i in model.i)
        else:
            return model.Results[g] == model.OMC
    
    model.Results_rule = Constraint( model.g, rule = Results_rule)
