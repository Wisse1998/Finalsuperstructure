...
# =============================================================================
        ...
        InTrans11_data = {(d,k) : value(InTrans11) for (d, k), InTrans11 in model.InTrans11.items()}
        InTrans11_data = pd.DataFrame.from_dict(InTrans11_data, orient="index", columns=["variable value"])
        InTrans11_data = InTrans11_data.values.reshape(len(self.d),len(self.k)).transpose()
        InTrans11_data = pd.DataFrame(InTrans11_data)
        InTrans11_data.index = self.k
        InTrans11_data.index.name = 'k'
        InTrans11_data.columns = self.d
        InTrans11_data.columns.name = 'd'
        self.InTrans11_data = InTrans11_data    
        print(InTrans11_data.to_string())
        
        InTrans22_data = {(d,k) : value(InTrans22) for (d, k), InTrans22 in model.InTrans22.items()}
        ...
        InTrans222_data = {(k) : value(InTrans222) for k, InTrans222 in model.InTrans222.items()}
        InTrans222_data = pd.DataFrame.from_dict(InTrans222_data, orient="index", columns=["variable value"])
        InTrans222_data = InTrans222_data.values.reshape(len(self.k)).transpose()
        InTrans222_data = pd.DataFrame(InTrans222_data)
        InTrans222_data.index = self.k
        InTrans222_data.index.name = 'k'
        self.InTrans222_data = InTrans222_data
        print(InTrans222_data)
        print()
        
# =============================================================================
#         InTrans33_data = {(k) : value(InTrans33) for k, InTrans33 in model.InTrans33.items()}
#         InTrans33_data = pd.DataFrame.from_dict(InTrans33_data, orient="index", columns=["variable value"])
#         InTrans33_data = InTrans33_data.values.reshape(len(self.k)).transpose()
#         InTrans33_data = pd.DataFrame(InTrans33_data)
#         InTrans33_data.index = self.k
#         InTrans33_data.index.name = 'k'
#         self.InTrans33_data = InTrans33_data
#         print(InTrans33_data)
#         print()
# =============================================================================
        
# =============================================================================
#         InTrans33_data = {(d,k) : value(InTrans33) for (d, k), InTrans33 in model.InTrans33.items()}
#         InTrans33_data = pd.DataFrame.from_dict(InTrans33_data, orient="index", columns=["variable value"])
#         InTrans33_data = InTrans33_data.values.reshape(len(self.d),len(self.k)).transpose()
#         InTrans33_data = pd.DataFrame(InTrans33_data)
#         InTrans33_data.index = self.k
#         InTrans33_data.index.name = 'k'
#         InTrans33_data.columns = self.d
#         InTrans33_data.columns.name = 'd'
#         self.InTrans33_data = InTrans33_data    
#         print(InTrans33_data.to_string())
# =============================================================================
# =============================================================================
...
# =============================================================================
#         IntTrans_data = {(k) : value(IntTrans) for k, IntTrans in model.IntTrans.items()}
#         IntTrans_data = pd.DataFrame.from_dict(IntTrans_data, orient="index", columns=["variable value"])
#         IntTrans_data = IntTrans_data.values.reshape(len(self.k)).transpose()
#         IntTrans_data = pd.DataFrame(IntTrans_data)
#         IntTrans_data.index = self.k
#         IntTrans_data.index.name = 'k'
#         self.IntTrans_data = IntTrans_data
#         print(IntTrans_data)
#         print()
# =============================================================================
